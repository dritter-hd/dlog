...
public class LessThanFormula implements ComparisonFormula {
    ...
    public LessThanFormula(int left, int right) {
        if (left < 0) {
    ...
    public LessThanFormula(int left, ParameterValue<?> right) {
        if (left < 0) {
            throw new IllegalArgumentException("Left component number must not be negative");
        }
        if (right == null) {
    ...
    public LessThanFormula(ParameterValue<?> left, int right) {
        if (left == null) {
